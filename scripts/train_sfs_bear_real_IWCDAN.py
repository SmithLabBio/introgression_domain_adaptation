...
bear_dir = "/mnt/scratch/smithlab/megan/da_revision/empirical_bears_iwcdan"
outdir = "/mnt/scratch/smithfs/megan/da_revisions/bear_real_iwcdan"


def run_training(rep, max_lambda, batch, learn, enc_learn, disc_learn, epochs, importance_weights=False, iw_update_every=5, iw_min_weight=0.2, iw_max_weight=5.0, force=False):
    source_path = f"{sim_dir}/bear-secondary-contact-2-20000-train-sfs-norm.npz"
    target_path = f"{bear_dir}/combined_sfs.npz"
    ...
    out = f"{outdir}/batch{batch}.learn_{exp(learn)}.enc_learn_{exp(enc_learn)}.disc_learn_{exp(disc_learn)}.lambda_{max_lambda}_{epochs}_{importance_weights}/{rep}"
