...
outdir = "/mnt/scratch/smithfs/megan/da_revisions/pseudo_noiwcdan_v2"
...
def run_training(rep, max_lambda, batch, learn, enc_learn, disc_learn, target_num, target_prop, train, importance_weights=False, iw_update_every=5, iw_min_weight=0.2, iw_max_weight=5.0, force=False):
    ...
    subsampled_path = f"{outdir}/batch{batch}.learn_{exp(learn)}.enc_learn_{exp(enc_learn)}.disc_learn_{exp(disc_learn)}.lambda_{max_lambda}.target_{target_num}_{target_prop}_{train}_{importance_weights}/{rep}/general-secondary-contact-ghost-1-100-train-sfs-norm.subsampled.npz"
