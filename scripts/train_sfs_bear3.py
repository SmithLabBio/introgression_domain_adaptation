...
def run_training(format, rep, lamb, batch, learn, enc_learn, disc_learn, epochs):
    if format == "sfs":
    ...
    cdan_out = f"{outdir}/batch{batch}.learn-{exp(learn)}.enc_learn-{exp(enc_learn)}.disc_learn{exp(disc_learn)}.{format}/{rep}"
    ...
    plot_adapt_history(cdan_history, cdan_out)
