...
process train {
    ...
    input:
    tuple val(populations), val(replicate), val(batch_size), val(learn_rate), val(enc_learn_rate), val(disc_learn_rate)
    
    script:
    """
    /mnt/home/kc2824/popAI/kerry_evo/networks/train_sfs_bear_real3.py \
      --pops ${populations} \
      --rep ${replicate} \
      --batch ${batch_size} \
      --learn ${learn_rate} \
...
workflow {
  ...
  comb = populations.combine(replicate).combine(batch_size).combine(learn_rate).combine(enc_learn).combine(disc_learn)
  train(comb)
}
