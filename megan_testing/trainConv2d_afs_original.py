...
from src.models_afs_npy import getEncoder, getTask
from tensorflow.keras.utils import to_categorical
import os

outdir="results/afs/original/ghost/"
...
np.savetxt(os.path.join(outdir, "test_cm.txt"), predict_afs_npy(finetunig, test, labels_test, os.path.join(outdir, "test_roc.txt")), fmt="%1.0f")
np.savetxt(os.path.join(outdir, "bgs_cm.txt"), predict_afs_npy(finetunig, bgs, labels_bgs, os.path.join(outdir, "bgs_roc.txt")), fmt="%1.0f")
np.savetxt(os.path.join(outdir, "ghost_cm.txt"), predict_afs_npy(finetunig, ghost, labels_ghost, os.path.join(outdir, "ghost_roc.txt")), fmt="%1.0f")
