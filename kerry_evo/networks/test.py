...
def test(json_path, source_path, target_path, outdir, epoch=None, epoch_selector=None):
    ...
    outdir = f"{data['outdir']}/{outdir}"
    if not os.path.isdir(outdir):
        os.makedirs(outdir)

    if epoch_selector:
        history = f"{data['outdir']}/history.csv"
        epoch = eval(epoch_selector)

    models = import_module(data["ModelFile"]) 
    ...
    model.load_weights(f"{data['outdir']}/checkpoints/{epoch}.hdf5")
