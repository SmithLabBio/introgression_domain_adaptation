...
outdir = "/mnt/scratch/smithfs/cobb/popai/bear1"

def run_training(meth, rep, batch, learn, enc_learn, epochs):
    ...
    out_prefix = f"{outdir}/batch{batch}.learn_{exp(learn)}.enc_learn_{exp(enc_learn)}"
    out_suffix = f"/{rep}"
    if meth == "finetune":
